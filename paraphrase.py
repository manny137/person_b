        ...
        if not has_passive:
            ...
            for sent in doc.sents:
                ...
                if subj and verb and obj:
                    ...
                    aux = "was"
                    if verb.tag_ in ["VBP", "VBZ"]: # is/are
                        aux = "is"

                    # Simple participle
                    participle = verb.lemma_ + "ed"
                    if verb.lemma_.endswith("e"):
                        participle = verb.lemma_ + "d"

                    # Use noun chunks for better phrases
                    subj_text = next((c.text for c in sent.noun_chunks if c.start <= subj.i < c.end), subj.text)
                    obj_text = next((c.text for c in sent.noun_chunks if c.start <= obj.i < c.end), obj.text)

                    passive_form = f"{obj_text} {aux} {participle} by {subj_text}"
                    result = result.replace(sent.text.strip(), passive_form)
                    # Only do it for the first sentence that matches
                    break
